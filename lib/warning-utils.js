...
function diffWarnings(previousWarnings, nextWarnings) {
  const previous = new Map((previousWarnings || []).map(warning => [warning.id, warning]));
  const next = new Map((nextWarnings || []).map(warning => [warning.id, warning]));
  ...
  for (const [id, warning] of next) {
    const previousWarning = previous.get(id);

    if (!previousWarning) {
      issued.push(warning);
    } else if (warningSignature(previousWarning) !== warningSignature(warning)) {
      updated.push(warning);
    }
  }

  for (const [id, warning] of previous) {
    if (!next.has(id)) {
      ended.push(warning);
    }
  }

  return { issued, updated, ended };
}
